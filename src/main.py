...
class JWTGenerator(object):
    """
    Creates and signs a JWT with the specified private key file, 
    username, and account identifier. The JWTGenerator keeps the
    generated token and only regenerates the token if a specified 
    ...
    def __init__(self, account: Text, user: Text, private_key_file_path: Text, private_key_passphrase: Text,
        ...
        """
        __init__ creates an object that generates JWTs for the specified user, 
        account identifier, and private key.
        ...
        with open(self.private_key_file_path, 'rb') as pem_in:
            ...
            try:
                # Try to access the private key with a passphrase as a parameter.
                self.private_key = load_pem_private_key(pemlines, self.private_key_passphrase.encode(), default_backend())
            except TypeError:
